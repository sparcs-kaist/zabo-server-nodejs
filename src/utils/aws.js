...
const bucket =
  ...
  (process.env.NODE_ENV === "production"
    ? "sparcs-kaist-zabo-prod"
    : "sparcs-kaist-zabo-dev");
...
export const zaboUpload = multer({
  storage: multerS3({
    ...
    key: (req, file, cb) => {
      ...
      zaboCount += 1;
      const fullPath = `zabo/zabo-${filename}${path.extname(
        file.originalname,
      )}`;
      cb(null, fullPath);
    },
...
export const profileUpload = type =>
  multer({
    storage: multerS3({
      ...options,
      key: (req, file, cb) => {
        const filename = Date.now().toString();
        const fullPath = `profile/${type}-${filename}${path.extname(
          file.originalname,
        )}`;
        cb(null, fullPath);
      },
