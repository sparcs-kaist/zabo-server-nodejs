...
export const getZabo = ash(async (req, res) => {
  ...
  let newVisit;
  if (
    req.get("User-Agent").length > 20 &&
    (!req.session[zaboId] || moment().isAfter(req.session[zaboId]))
  ) {
    newVisit = true;
  ...
  if (newVisit) {
    ...
    zabo = await Zabo.findByIdAndUpdate(
      zaboId,
      { $inc: { views: 1, effectiveViews: 1 } },
      { new: true },
  ...
  if (!zabo) {
    logger.zabo.error(
      "get /zabo/ request error; 404 - zabo does not exist - id: %s",
      zaboId,
    );
  ...
  if (self) {
    zaboJSON.isMyZabo = self.groups.some(group =>
      group.equals(zaboJSON.owner._id),
    );
    zaboJSON.owner.following = self.followings.some(following =>
      following.followee.equals(zaboJSON.owner._id),
    );
    if (zaboJSON.isMyZabo)
      zaboJSON.createdBy = await User.findById(zaboJSON.createdBy, "username");
...
export const listMagamImbakList = ash(async (req, res) => {
  ...
  const zabos = await Zabo.find({
    schedules: { $elemMatch: { startAt: { $lt: nextWeek, $gt: cur } } },
  });
...
export const listZabos = ash(async (req, res, next) => {
  ...
  if (relatedTo) {
    const zabo =
      isValidId(relatedTo) && (await Zabo.findOne({ _id: relatedTo }));
    if (!zabo) {
      logger.zabo.error(
        "get /zabo/list request error; 404 - zabo does not exist",
      );
      return res.status(404).json({
...
export const pinZabo = ash(async (req, res) => {
  ...
  logger.zabo.info(
    `post /zabo/pin request; zaboId: ${zaboId}, by: ${self.username} (${self.sso_sid})`,
  );

  // currently user has only 1 boardObject!
  await self.populate("boards").execPopulate();
...
export const likeZabo = ash(async (req, res) => {
  ...
  logger.zabo.info(
    `post /zabo/like request; zaboId: ${zaboId}, by: ${self.username} (${self.sso_sid})`,
  );
