...
const getGroupProfile = ash(async (req, res) => {
  ...
  let following = false;
  if (self) {
    const selfMember = group.members.find(member =>
      member.user.equals(self._id),
    );
...
const getUserProfile = ash(async (req, res) => {
  ...
  actions.push(Board.findById(boardId));
  actions.push(
  ...
  if (self) {
    ...
    if (!own)
      following = user.followers.some(follower => follower.equals(self._id));
  }
  const result = {
...
export const followController = ash(async (req, res) => {
  ...
  const prevFollow = self.followings.find(following =>
    following.followee.equals(profile._id),
  );
  if (prevFollow) {
    self.followings.pull(prevFollow);
    profile.followers = profile.followers.pull(self._id);
    await Promise.all([self.save(), profile.save()]);
    return res.send({
      following: false,
